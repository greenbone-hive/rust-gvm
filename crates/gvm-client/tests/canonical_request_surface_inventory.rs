...
fn every_public_request_surface_has_an_explicit_disposition() {
    ...
    assert_eq!(ledger.len(), 998, "#660 disposition ledger total drifted");
    assert_eq!(counts.get("transitional"), Some(&150));
    assert_eq!(counts.get("canonical-request"), Some(&345));
    assert_eq!(counts.get("removed"), Some(&380));
