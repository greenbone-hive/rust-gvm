...
Typed execution is developed on protected `next`. A downstream-ready release
is produced only after the bounded change set is reviewed and promoted to
protected `main`, the workspace version and lockfile are updated through a
separate reviewed pull request, and the orchestrated release workflow publishes
and verifies the tag and artifacts.
