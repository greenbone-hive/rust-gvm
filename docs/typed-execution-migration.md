...
The bounded typed-execution change set is promoted to protected `main` through
a reviewed pull request. A downstream-ready release is produced only after the
workspace version and lockfile are updated through a separate reviewed pull
request and the orchestrated release workflow publishes and verifies the tag
and artifacts.
