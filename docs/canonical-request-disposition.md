...
After the bounded core and system-discovery migration in #663, the ledger
contains exactly 1024 rows: 32 `transitional`, 400 `canonical-request`, 454
`removed`, 126 `retained-construction`, and 12 `frozen-ticket`. Aggregate,
pre-authentication, feature, feed, help, read-only system, system-report, and
version request/facade surfaces contain no transitional rows. Mutations, user
settings, wizard execution, and trashcan cleanup remain explicitly deferred to
#664. The inventory test recognizes projection requests generated by the local
request macro and protects both these counts and source correspondence.
