...
After the bounded report lifecycle/structured migration in #661 and the
drill-down/export migration in #662, the ledger contains exactly 1017 rows: 89
`transitional`, 385 `canonical-request`, 420 `removed`, 111
`retained-construction`, and 12 `frozen-ticket`. The complete
`commands/reports.rs` and typed report-facade scope contains no transitional
rows. The inventory test recognizes the projection requests generated by the
local request macro and protects both these counts and source correspondence.
