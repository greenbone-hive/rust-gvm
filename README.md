...
Tagged release builds publish CycloneDX 1.5 JSON and XML SBOMs generated by
`cargo-cyclonedx`. The JSON artifacts are post-processed to add CC0 document
licensing, build lifecycle metadata, and supplier hints before `sbomqs` scoring
and packaging. CPEs are intentionally left untouched unless a reliable package
mapping is available.

> [!NOTE]
> **Releases** are managed via the repository's
> [orchestrated release workflow](.github/workflows/release-orchestrated.yml).
> Pre-releases use the same exact-revision release path with a SemVer pre-release
> version. This repository does not currently run a scheduled nightly workflow.
...
| [`gvm-gmp`](crates/gvm-gmp/) | Complete typed GMP requests, responses, and capability metadata (22.4–22.8+) | ✅ Implemented |
...
├─────────────────────────────────┤
│         gvm-gmp                 │  Canonical requests, codecs, responses
├─────────────────────────────────┤
...
All other supported non-ticket families follow the same canonical request
contract; ticket support remains deliberately frozen. Raw `send` and `call`
...
`SshConnection` is available when the direct `gvm-connection` dependency enables
its `ssh` feature. Transport features are owned by `gvm-connection`; they are not
features on `gvm-client`.
...
`TlsConnection` is available when the direct `gvm-connection` dependency enables
its `tls` feature.
...
The mock library and CLI default to GMP 22.7. Select 22.8 explicitly when
testing agent, container-scanning, or other feature-enabled 22.8 surfaces.
...
| **Fixture** | Returns pre-built realistic GMP XML from the built-in fixture library |
| **Stateful** | Bounded in-memory resource behavior with authentication, task lifecycle, and resource relationships; see the support matrix for exact qualifications |
| **Scenario** | Library-only scripted command→response playback with strict or lenient matching |

The standalone CLI accepts `echo`, `fixture`, and `stateful`; configure scenario
mode through `MockGmpServer::builder().scenario(...)`.

### Features

- **4 server modes** — echo, fixture, bounded stateful behavior, and library-only scenario playback
- **Unix socket, TCP, and feature-gated TLS listeners** — TLS uses a generated certificate and can optionally require a trusted client certificate; the default-feature release image exposes Unix/TCP only
...
The mock server is validated against [python-gvm](https://github.com/greenbone/python-gvm) in CI, exercising Unix-socket target/task/host/note flows plus anonymous TLS and mutual-TLS reconnect/authentication. This protects migration and client interoperability while rust-gvm continues to model GMP/GVMD behavior directly; it is not a real-gvmd conformance claim.
...
# Check every transport and crate feature
cargo check --workspace --all-features

# Run the default workspace tests
...
- Rust 1.89+ (MSRV)
- Python 3.10+ with the locked integration requirements (CI uses Python 3.12)
...
| **Release** | `v*` tag push | Locked all-feature tests, five binary builds, multi-platform GHCR image, SBOM bundle, checksums, attestations, and publication verification |

After a successful test job on a `main` push, CI dispatches the external E2E
workflow with `rust-gvm-ref` set to the exact `${{ github.sha }}`. Pull requests
do not trigger that external workflow.

### Pre-built Binaries

Download tagged artifacts from
[GitHub Releases](https://github.com/greenbone-hive/rust-gvm/releases). There is
no current rolling-nightly publication workflow.
...
`ghcr.io/greenbone-hive/gvm-mock-server:<v-prefixed-tag>` for Linux amd64 and
arm64. The workflow publishes only the immutable release tag, not `latest`.
